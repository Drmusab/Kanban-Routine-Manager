...
# Run migrations + version stamping
...
- **Vaults**: `vault_data` volume mounted at `/app/vaults` (`VAULT_PATH`) for markdown, templates, and attachments manifest metadata.
- **Database**: `db_data` volume mounted at `/app/data` (`DATABASE_PATH`) to keep the SQLite index portable across updates.
- **Configuration**: `app_config` volume for settings files and templates consumed by the Electron preload bridge.
- **Attachments**: `attachments` volume for uploaded assets referenced by notes, tasks, and PARA records.
- **Backups**: `backups` volume captures `.tar.gz` archives generated by `npm run backup` inside the `api` container.
